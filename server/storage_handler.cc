...
void StorageHandler::onRequest(std::unique_ptr<HTTPMessage> headers) noexcept {
    auto params_str =
        headers->getHeaders().getSingleOrEmpty("Data-Audit-Params");
    ...
    out_file_.open(file_name, std::ofstream::binary);
}

void StorageHandler::onBody(std::unique_ptr<folly::IOBuf> body) noexcept {
    out_file_.write(reinterpret_cast<const char*>(body->data()),
                    body->length());
}
